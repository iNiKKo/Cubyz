...
const Context = struct {
	...
	fn generateGradient(self: Context, x: i32, y: i32, i: i32, resolution: u32) f32 {
		var seed: u64 = self.l1*%@as(u32, @bitCast(x)) +% self.l2*%@as(u32, @bitCast(y)) +% self.l3*%@as(u32, @bitCast(i)) +% resolution;
	...
	fn calculateGridPoints(self: *Context, allocator: NeverFailingAllocator, x: i32, y: i32, _width: u31, _height: u31, scale: u31) void {
		...
		self.xGridPoints = Array2D(f32).init(allocator, width/scale + 3, height/scale + 3);
		self.yGridPoints = Array2D(f32).init(allocator, width/scale + 3, height/scale + 3);
		var numX: u31 = 0;
